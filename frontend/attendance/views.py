...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        # Proxy authentication to FastAPI backend
        try:
            import requests
            resp = requests.post(
            ...
            if resp.status_code == 200:
                ...
                if token:
                    ...
                    # Ensure there is a corresponding Django user for session auth gates
                    User = get_user_model()
                    ...
                    # Persist FastAPI token for frontend JS via cookie
                    try:
                        resp.set_cookie('fastapi_access_token', token, samesite='Lax')
                    except Exception:
                        pass
                    return resp
                ...
                messages.error(request, f'Login failed: {resp.text}')
        except Exception as e:
            messages.error(request, f'Error contacting authentication service: {str(e)}')
